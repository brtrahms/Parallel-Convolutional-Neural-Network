...
This program is based on code owned by Alan Wolfe

Found at:
Website: https://blog.demofox.org/2017/03/15/neural-network-recipe-recognize-handwritten-digits-with-95-accuracy/
Repo: https://github.com/Atrix256/MNIST1

This code trains, tests, and reports performance metrics for a Convolutional Neural Network learning to accurately classify 
handwritten digits from the MNIST dataset.

Brandon C Trahms - 5/18/2022

This code has been altered to take advantage of the many SIMD operations performed(e.g Matrix Vector Multiplication, Bias Additions, Weight Adjustments) 
by speeding them up using OpenMP to make parallel.
*/
	...
	void Train (const CMNISTData& trainingData, size_t miniBatchSize, float learningRate)
	{
		...
		while (trainingIndex < trainingData.NumImages())
		{
			...
			while (miniBatchIndex < miniBatchSize && trainingIndex < trainingData.NumImages())
			{
				...
				// Since all of these loops run as a SIMD operation, they can all be made parallel
...
#pragma omp parallel for num_threads(THREADS)
	...
	uint8 ForwardPass (const float* pixels, uint8 correctLabel)
	{
		...
		// Since this Vector Matrix Multiplication is a SIMD operation, it can be made parallel
#pragma omp parallel for num_threads(THREADS)
...
private:
	...
	void BackwardPass (const float* pixels, uint8 correctLabel)
	{
		...
		// Since this Weight/Bias adjustment is a SIMD operation, it can be made parallel
#pragma omp parallel for num_threads(THREADS)
		for (size_t neuronIndex = 0; neuronIndex < OUTPUT_NEURONS; ++neuronIndex)
		{
